...
function font_end_js_control()
{
    ...
    $iro_opt = array(
        ...
        'yiyan_api' => empty(iro_opt('yiyan_api')) ? array("https://v1.hitokoto.cn/", "https://api.nmxc.ltd/yiyan/") : json_decode(iro_opt('yiyan_api')),
    ...
    if (iro_opt('random_graphs_options') == 'external_api') {
        ...
        $iro_opt['cover_api'] = rest_url('sakura/v1/gallery') . '?img=' . (wp_is_mobile() ? 'l' : 'w');
    }

    if (is_singular()) {
        $annotations = get_post_meta(get_the_ID(), 'iro_chatgpt_annotations', true);
        $iro_opt['have_annotation'] = check($annotations);
        $iro_opt['extract_article_highlight'] = iro_opt('extract_article_highlight_from_feature', false) ? true : false;
        $iro_opt['post_theme_color'] = var_post_theme_color();
        $iro_opt['post_cover_as_bg'] = check(iro_opt('post_cover_as_bg', false) && iro_opt('site_bg_as_cover', true));
        $iro_opt['post_feature_img'] = get_post_thumbnail_id(get_the_ID()) ? get_the_post_thumbnail_url(get_the_ID(), 'full') : '';
        if (!empty($annotations)) {
            $iro_opt['page_annotation'] = $annotations;
        }
    }

    if (iro_opt('live_search')) {
        $iro_opt['live_search'] = true;
        $iro_opt['live_search_preview'] = check(iro_opt('live_search_preview', true));
    }
    ...
    if (iro_opt('aplayer_server') != 'off') {
        ...
        $iro_opt['meting_api_url'] = !empty(iro_opt('custom_music_api'))
            ? iro_opt('custom_music_api')
            : rest_url('sakura/v1/meting/aplayer');
    }
    if (iro_opt('code_highlight_method', 'hljs') == 'prism') {
        $iro_opt['code_highlight_prism'] = array(
        ...
        if ($theme_light) {
            $iro_opt['code_highlight_prism']['theme'] = $theme_dark
                ? array('light' => $theme_light, 'dark' => $theme_dark)
                : array('light' => $theme_light);
        } elseif ($theme_dark) {
            $iro_opt['code_highlight_prism']['theme'] = array('dark' => $theme_dark);
        }
    }
    ...
    if (iro_opt('theme_darkmode_auto')) {
        $iro_opt['dm_strategy'] = iro_opt('theme_darkmode_strategy', 'time');
    }

    wp_add_inline_script(
        'app',
        'var _iro = ' . wp_json_encode($iro_opt, JSON_NUMERIC_CHECK | JSON_UNESCAPED_UNICODE),
        'before'
    );
}
